...
export default function MapLibreMinimap({
  ...
  useEffect(() => {
    ...
    let styleToUse = MAP_STYLES.voyager; // Default
  ...
  useEffect(() => {
    ...
    let styleToUse = MAP_STYLES.voyager;
  ...
  return (
    <div
      ...
      style={{
        ...
        // Apply high contrast filter for GTA style to mimic the blueprint look
        // We invert a Voyager map (defined streets) to get visible streets on dark background
        // brightness(3) forces the grey lines to become white
        filter: mapStyle === 'gta' ? 'invert(1) grayscale(1) brightness(3) contrast(1.3)' : 'none'
      }}
    />
  );
}
